...
- Run the repository CI-equivalent validation locally: `python -m pytest --cov --cov-report=term-missing`. Also run `git diff --check` and inspect the final diff for secrets, unrelated changes, and generated-file contracts.
...
- Do not submit a PR as ready if a required test, lint check, or build fails. If validation is blocked, keep the PR draft and state the exact blocker and command output in the PR body.
